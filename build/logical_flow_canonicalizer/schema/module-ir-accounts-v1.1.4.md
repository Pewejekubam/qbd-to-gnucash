...
#       description: List of parsed account records (from dispatcher; no direct .IIF parsing)
#     input_path:
#       type: str
#       description: Original source file path (traceability)
#     output_dir:
#       type: str
...
#     mapping_config:
#       type: Dict[str, Any]
#       description: Account mapping configuration (see Mapping File Schema)
#     extra_config:
#       type: Optional[Dict[str, Any]]
#       description: Additional configuration (reserved for agentic/override use)
...
#     """Entry point for processing QBD !ACCNT list (from dispatcher payload only)."""
# ```
# exceptions:
...
# ### load_mapping (from accounts_mapping.py v1.3.2)
# ```python
# def load_mapping(user_mapping_path: Optional[str] = None) -> Dict[str, Any]:
#     """Load and merge mapping files for QBD to GnuCash account types, with text workflow orchestration."""
# ```
# exceptions:
...
# ### run_validation_pass (from accounts_validation.py v1.1.0)
# ```python
...
# - Valid QuickBooks account types (from dispatcher; no direct .IIF parsing)
