...
def build_gnucash_accounts(accounts_data, mapping):
    ...
    for account in accounts_data:
        ...
        # Special handling for AR/AP business accounts
        full_name = f"{destination}:{name}" if destination else name
        if name.strip().upper() == "ACCOUNTS RECEIVABLE" and full_name.upper().startswith("ASSETS:"):
            gnucash_type = "ASSET"
        elif name.strip().upper() == "ACCOUNTS PAYABLE" and full_name.upper().startswith("LIABILITIES:"):
